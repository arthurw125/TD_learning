...
# Transfer on Purple
def Transfer_Task(WM_Objs,state_size,nepisodes,stat_window,ep_0,task):
    #n = 512
    beginTime = time.time()
    num_agents = 100
    n = 1024
    ...
    t1 = ""
    # testing
    #print('epsilon Transfer test before:',agent.epsilon,i_gate.epsilon,o_gate.epsilon)
    #handhold = 400000
    anneal = ep_0
    ...
    all_diff_array = [] # stores the step differentials for each episode, used for data storage
    memory_states = []
    ...
    test_flag = False
    # reset epsilon for agents and gates
    agent.epsilon,i_gate.epsilon,o_gate.epsilon = 0.1, 0.1, 0.1
    # testing
    #print('epsilon Transfer test after:',agent.epsilon,i_gate.epsilon,o_gate.epsilon)
    while episode < nepisodes+1:
        ...
        if color != nothing:    
            ...
            #print(step_diff)
            all_diff_array.append(step_diff) # stores all step differentials
    ...
    fname = 'Transfer_step_diff_'+task+'_testing.dat'
    fname2 = 'Transfer_step_diff_red_'+task+'_testing.dat'
    fname3 = 'Transfer_step_diff_green_'+task+'_testing.dat'
    fname4 = 'Transfer_step_diff_purple_'+task+'_testing.dat'
    fname5 = 'Transfer_memory_states_'+task+'_testing.dat'
    fname6 = 'Transfer_step_diff_'+task+'_training.dat'
    ...
    store_weights_csv_row(fname6,all_diff_array) # stores step differential as csv file (appends)
    endTime = time.time()
...
#LTM_obj2, agent2, i_gate2, o_gate2, WM2 = IO_gate_model([5,5],100000,1000)
