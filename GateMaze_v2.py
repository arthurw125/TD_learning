...
class output_gate:
    ...
    def __init__(self,W,gate_states,eligibility,bias=1,lrate=.4,gamma=.9,td_lambda=.9):
        self.W = W
    ...
    def gate_action(self,role,cur_state,open):
        my_state1 = hrr.convolve(role,cur_state)
        my_state2 = hrr.convolve(my_state1,self.gate_states)
        #print(my_state.shape)
        values = np.dot(my_state2,self.W)
        #print(values)
        sm = softmax(values)
        action = np.argmax(sm)
        if random.random() < self.epsilon:
        ...
        if open:
            action = 1
        #return action, values[action], my_state2[action] # return action and (value given action)
        return action, values[action], hrr.convolve(my_state1,self.gate_states[action])
    
    def eligibility_trace_update(self,state_action):
        #s_a = hrr.convolve(state_vec,action_vec)
        self.eligibility = state_action + self.td_lambda*self.eligibility
    
    def set_eligibility_zero(self,n):
        self.eligibility = np.zeros(n)
...
class agent:
    ...
    def __init__(self,W,actions,eligibility,bias=1,lrate=.4,gamma=.9,td_lambda=.9):
        self.W = W
    ...
    def agent_action(self,cue,cur_state,wm_out):
        my_state1 = hrr.convolve(wm_out,hrr.convolve(cue,cur_state))
        my_state2 = hrr.convolve(my_state1,self.actions)
        values = np.dot(my_state2,self.W)
        sm = softmax(values)
        action = np.argmax(sm)
        if random.random() < self.epsilon:
            action = random.randrange(0,2)
        return action, values[action], my_state2[action]
        #return action, values[action], hrr.convolve(my_state1,self.actions[action])
